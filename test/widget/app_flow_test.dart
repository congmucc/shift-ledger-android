...
void main() {
  ...
  testWidgets('webdav sheet makes auto sync trigger timing explicit', (
    ...
    expect(find.textContaining('打开 App 或账本变化后自动检查'), findsOneWidget);

    await tester.ensureVisible(find.byType(SwitchListTile));
    await tester.tap(find.byType(SwitchListTile));
    await tester.pumpAndSettle();

    expect(find.textContaining('1 小时内不重复上传'), findsOneWidget);
  });

  testWidgets('webdav save action sits above the compact auto backup section', (
    tester,
  ) async {
    final state = LedgerState.empty(now: DateTime(2026, 5, 13))
      ..updateWebDavConfig(
    ...
    expect(
      tester.getTopLeft(find.text('保存配置')).dy,
      lessThan(tester.getTopLeft(find.text('自动云备份')).dy),
    );
  });
